...
class GestureDemo:
    ...
    def set_new_cursor_position(self, newpos):
        ...
        # print "gesturedemo.py: trying to set", oldpos, newpos
    ...
    def update_display(self,xyposition,zpos,message=None):
        ...
        print "gesturedemo.py:final pos:", xypos_final
        ...
        text = self.font.render(message, 1, (10, 10, 10))
        textpos = text.get_rect()
        textpos.centerx = self.background.get_rect().centerx
...
def main():
    GD = GestureDemo()
    num_parameters = GD.square_size-3
    cursor_positions = [[.2,.1], [.4,.2], [.6,.5]]

    # set up the event loop ...
    currsize = 0
    currpos = 0
    statements = ["num"+str(val) for val in range(100)]
    i = 0
    start_time = time.clock()
    while time.clock() < start_time + 5:
        event = pg.event.poll()
        GD.update_display(cursor_positions[currpos],currsize, statements[i])
